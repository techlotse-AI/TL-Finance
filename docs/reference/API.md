...
| `GET, POST` | `/api/optimize/wealth/plans`, `PUT, DELETE /api/optimize/wealth/plans/{id}` | Persisted wealth-plan configurations (shared JSON config, `wealthPlanConfigSchema` v1, including an optional descriptive `purpose`) driving the projection and drawdown views |
