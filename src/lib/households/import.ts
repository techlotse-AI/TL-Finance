...
import type { HouseholdImport } from "@/lib/households/export";

export async function importHousehold(
  transaction: Prisma.TransactionClient,
  userId: string,
  input: HouseholdImport,
) {
  ...
  for (const pocket of input.accountPockets) {
    ...
    if (!accountId) throw new ApiError(400, "invalid_import_reference", "Currency route references an unknown account.");
  ...
  for (const source of input.incomeSources) {
    ...
    await transaction.incomeSource.create({
      data: {
        ...
        allocations: {
          create: source.allocations.map((allocation) => {
            const accountPocketId = pocketMap.get(allocation.accountPocketId);
            if (!accountPocketId) throw new ApiError(400, "invalid_import_reference", "Income allocation references an unknown currency route.");
            return {
              householdId: created.id,
              accountPocketId,
  ...
  for (const transfer of input.plannedTransfers) {
    ...
    if (!fromAccountPocketId || !toAccountPocketId) throw new ApiError(400, "invalid_import_reference", "Transfer references an unknown currency route.");
  ...
  for (const item of input.budgetItems) {
    ...
    if (item.paidFromAccountPocketId && !paidFromAccountPocketId) throw new ApiError(400, "invalid_import_reference", "Budget item references an unknown funding route.");
    if (item.paidToAccountPocketId && !paidToAccountPocketId) throw new ApiError(400, "invalid_import_reference", "Budget item references an unknown destination route.");
